#!/usr/bin/env python3
"""One-off: build the section/lesson skeleton for course 183088 (PAID
"Профессия SMM: продвинутый курс" — tier-1 practice course, built on top of
free course 183078). One project-based structure, practical tasks instead
of quizzes. Mirrors build_183077_skeleton.py.
"""
import json
import os
import sys
...
SKELETON = [
    ...
    ("Аудит и стратегия", [
        "Что будет в модуле «Аудит и стратегия»",
        "Реальный аккаунт: с чего начинается аудит",
        "От аудита к позиционированию: что писать и для кого",
        "Практика: провести аудит своего или чужого аккаунта",
    ]),
    ("Контент-план", [
        "Что будет в модуле «Контент-план»",
        "Система постов вместо разовых публикаций",
        "Форматы, которые двигают вовлечённость",
        "Практика: собрать контент-план на месяц",
    ]),
    ("Продвижение", [
        "Что будет в модуле «Продвижение»",
        "Бесплатные способы роста аккаунта",
        "Платное продвижение: если бюджет есть",
        "Практика: составить план продвижения",
    ]),
    ("Отчёт с рекомендацией", [
        "Что будет в модуле «Отчёт с рекомендацией»",
        "Структура отчёта для клиента или руководителя",
        "Как сформулировать рекомендацию с конкретной цифрой",
        "Практика: написать отчёт по своему проекту",
    ]),
    ("Комментарии и репутация", [
        "Что будет в модуле «Комментарии и репутация»",
        "Токсичные комментарии: как отвечать, а когда молчать",
        "Кризис-коммуникация: если что-то пошло не так",
        "Практика: подготовить ответы на сложные комментарии",
    ]),
    ("Что дальше", [
        "Итоги курса",
        "Что дальше",
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "_183088_skeleton_created.json")
