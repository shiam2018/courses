...
// intro video for course 183082 ("Профессия Zero-code разработчик" — paid
// tier-1 practice course, 5900₽, shared upsell for 183075 and 183083).
  ...
  build: () => {
    ...
    const html = `
      <div class="title-wrap">
        ...
        <h1 class="big" style="color:#fff;">Профессия Zero-code разработчик</h1>
        <p class="sub">Один реальный сайт или приложение: от идеи до публикации на FlutterFlow и конструкторах.</p>
      </div>
    `;
    ...
    const html = `
      ...
      <div class="avtor-content">
        ...
        <ul class="list-plain" style="margin-top:12px;font-size:13px;">
          ...
          <li>Провожу проекты от идеи до публикации на no-code конструкторах</li>
          <li>Курс собран вокруг одного реального проекта, а не разрозненных советов</li>
        </ul>
      </div>
    `;
...
slides.push({
  out: 'l03-dlya-kogo',
  build: () => contentSlide('Для кого этот курс', 'Для тех, кто хочет довести продукт до публикации целиком', `
    <ul class="list-plain" style="margin-top:14px;max-width:820px;">
      <li>Для предпринимателей и продакт-менеджеров без своего разработчика</li>
      <li>Для тех, кто прошёл «Сайт без кода» или «Приложение без кода» и хочет применить это на практике</li>
      <li>Продолжение обоих бесплатных курсов — работает независимо от того, какой из них вы проходили</li>
      <li>Платный курс, 5900 ₽, 4-5 часов</li>
    </ul>
  `, 820),
});
...
slides.push({
  out: 'l04-programma',
  build: () => contentSlide('Программа курса', 'Один сквозной проект от макета до публикации', stepsNum([
    { h: 'От идеи к макету', p: 'Выбор инструмента, экраны и путь пользователя' },
    { h: 'Интерфейс и логика', p: 'Сборка блоков, условия и переходы' },
    { h: 'Данные и интеграции', p: 'Реальный источник данных, формы, аккаунты' },
    { h: 'Тестирование и доработка', p: 'Реальные пользователи, частые no-code ошибки' },
    { h: 'Публикация и что дальше', p: 'Запуск и метрики после него' },
  ]), 820),
});
  ...
  build: () => wrap('', `
    <div class="content-pad">
      ...
      <ul class="list-plain" style="margin-top:14px;max-width:820px;">
        ...
        <li>Не привязан к устаревающим скриншотам одного интерфейса — принципы работают в любом конструкторе</li>
        <li>Отдельный модуль о типичных ошибках именно no-code проектов</li>
      </ul>
      ${highlightBox('Практика', 'К концу курса у вас на руках опубликованный сайт или приложение, а не только теория о конструкторах.')}
    </div>
  `, highlightCss),
});
...
slides.push({
  ...
  build: () => contentSlide('Как лучше проходить', 'Берите реальную идею и проходите модуль за модулем', `
    <ul class="list-plain" style="margin-top:20px;max-width:820px;font-size:15px;">
      ...
      <li>В конце — опубликованный продукт, а не только план на бумаге</li>
    </ul>
  `, 820),
});
...
slides.push({
  ...
  build: () => recapSlide({
    title: 'В конце курса',
    cards: [
      { heading: 'Модули курса', items: ['От идеи к макету', 'Интерфейс и логика', 'Данные', 'Тестирование', 'Публикация'] },
      { heading: 'Что получите', items: ['Макет и структуру своего проекта', 'Собранный интерфейс с данными', 'Опубликованный сайт или приложение'] },
      { heading: 'Дальше', text: 'Курс самостоятельный — навык создания продукта без кода не привязан к программе по маркетингу.' },
    ],
  }),
});
