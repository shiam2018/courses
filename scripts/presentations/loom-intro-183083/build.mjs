...
// intro video for course 183083 ("Приложение без кода: конструкторы для
  ...
  build: () => {
    ...
    const html = `
      <div class="title-wrap">
        ...
        <h1 class="big" style="color:#fff;">Приложение без кода: конструкторы для начинающих</h1>
        <p class="sub">От планирования экранов до публикации своего приложения без программиста.</p>
      </div>
    `;
    ...
    const html = `
      ...
      <div class="avtor-content">
        ...
        <ul class="list-plain" style="margin-top:12px;font-size:13px;">
          ...
          <li>Собирал прототипы приложений на no-code конструкторах для проверки идей</li>
          <li>Курс — практический разбор пути от идеи до опубликованного приложения</li>
        </ul>
      </div>
    `;
...
slides.push({
  out: 'l03-dlya-kogo',
  build: () => contentSlide('Для кого этот курс', 'Для тех, кто хочет собрать приложение без найма разработчика', `
    <ul class="list-plain" style="margin-top:14px;max-width:820px;">
      <li>Для начинающих предпринимателей и продакт-менеджеров</li>
      <li>Для всех, кто хочет собрать первое приложение самостоятельно</li>
...
slides.push({
  out: 'l04-programma',
  build: () => contentSlide('Программа курса', 'От планирования экранов до публикации', stepsNum([
    { h: 'Конструктор приложений', p: 'Как это работает и как выбрать сервис' },
    { h: 'Планирование экранов', p: 'Из каких экранов строится приложение' },
    { h: 'Интерфейс и данные', p: 'Сборка блоков и подключение списков, форм' },
    { h: 'Простая логика без кода', p: 'Кнопки, условия, обратная связь' },
    { h: 'Публикация приложения', p: 'Тест на телефоне, стор или веб-версия' },
  ]), 820),
});
...
slides.push({
  ...
  build: () => wrap('', `
    <div class="content-pad">
      ...
      <h1 class="title" style="max-width:780px;">Принципы, которые не устаревают вместе с интерфейсом</h1>
      <ul class="list-plain" style="margin-top:14px;max-width:820px;">
        <li>Фокус на логике планирования экранов, а не на конкретных кнопках меню</li>
        <li>Разбор, когда нужны аккаунты пользователей, а когда — нет</li>
        <li>Честное сравнение публикации в сторе и веб-версии (PWA)</li>
      </ul>
      ${highlightBox('Практично', 'Принципы курса работают в любом no-code конструкторе, даже когда очередной сервис обновит интерфейс.')}
    </div>
  `, highlightCss),
});
...
slides.push({
  ...
  build: () => contentSlide('Как лучше проходить', 'Держите в голове свою идею приложения', `
    <ul class="list-plain" style="margin-top:20px;max-width:820px;font-size:15px;">
      <li>Если уже есть идея приложения — планируйте экраны прямо по ходу курса</li>
      <li>Не открывайте конструктор до модуля о планировании экранов</li>
...
slides.push({
  ...
  build: () => recapSlide({
    title: 'В конце курса',
    cards: [
      { heading: 'Модули курса', items: ['Конструктор приложений', 'Планирование экранов', 'Интерфейс и данные', 'Простая логика', 'Публикация'] },
      { heading: 'Что получите', items: ['Понимание, когда конструктор подходит', 'Спланированные экраны приложения', 'Опыт публикации в сторе или как PWA'] },
