...
// intro video for course 183096 ("Профессия продакт-маркетолог" — paid
  ...
  build: () => {
    ...
    const html = `
      <div class="title-wrap">
        ...
        <h1 class="big" style="color:#fff;">Профессия продакт-маркетолог</h1>
        <p class="sub">Один сквозной запуск продукта: от позиционирования до отчёта перед бизнесом.</p>
      </div>
    `;
    ...
    const html = `
      ...
      <div class="avtor-content">
        ...
        <ul class="list-plain" style="margin-top:12px;font-size:13px;">
          ...
          <li>Каждый день формулирую позиционирование, выбираю каналы запуска и защищаю результат перед командой</li>
          <li>Курс собран вокруг одного реального цикла запуска, а не разрозненных советов</li>
        </ul>
      </div>
    `;
...
slides.push({
  out: 'l03-dlya-kogo',
  build: () => contentSlide('Для кого этот курс', 'Для тех, кто хочет провести запуск целиком, а не только его часть', `
    <ul class="list-plain" style="margin-top:14px;max-width:820px;">
      <li>Для маркетологов, которые растут в продакт-маркетолога</li>
      <li>Для начинающих продакт-маркетологов, которым нужен реальный кейс для портфолио</li>
      <li>Продолжение бесплатного курса «Product-маркетинг» — для тех, кто хочет применить основы на практике</li>
...
slides.push({
  out: 'l04-programma',
  build: () => contentSlide('Программа курса', 'Один сквозной запуск от позиционирования до отчёта', stepsNum([
    { h: 'Позиционирование продукта', p: 'Реальное отличие от конкурентов и сообщение' },
    { h: 'Go-to-market план', p: 'Выбор каналов и чек-лист запуска' },
    { h: 'Работа с командой', p: 'Обратная связь продукту, подготовка продаж' },
    { h: 'Тестирование гипотез запуска', p: 'Что тестировать и как читать данные' },
    { h: 'Отчёт и метрики', p: 'Как показать результат бизнесу' },
  ]), 820),
});
...
slides.push({
  ...
  build: () => wrap('', `
    <div class="content-pad">
      ...
      <h1 class="title" style="max-width:780px;">Практика на одном запуске вместо тестов</h1>
      <ul class="list-plain" style="margin-top:14px;max-width:820px;">
        <li>Каждый модуль заканчивается практическим заданием на вашем реальном продукте, а не квизом</li>
        <li>Задания идут последовательно: позиционирование → план → команда → тест → отчёт</li>
        <li>Готовые структуры позиционирования, плана запуска и отчёта — не абстрактные советы</li>
      </ul>
      ${highlightBox('Практика', 'К концу курса у вас на руках готовое позиционирование, go-to-market план, материал для продаж и отчёт по одному реальному запуску.')}
    </div>
  `, highlightCss),
});
...
slides.push({
  ...
  build: () => contentSlide('Как лучше проходить', 'Берите реальный продукт и проходите модуль за модулем', `
    <ul class="list-plain" style="margin-top:20px;max-width:820px;font-size:15px;">
      ...
      <li>В конце — готовый кейс для портфолио: позиционирование, план запуска, план теста, отчёт</li>
    </ul>
  `, 820),
});
...
slides.push({
  ...
  build: () => recapSlide({
    title: 'В конце курса',
    cards: [
      { heading: 'Модули курса', items: ['Позиционирование', 'Go-to-market план', 'Работа с командой', 'Тест гипотез', 'Отчёт и метрики'] },
      { heading: 'Что получите', items: ['Готовое позиционирование по своему продукту', 'Go-to-market план и материал для продаж', 'Отчёт, понятный бизнесу'] },
      { heading: 'Дальше', text: 'Программа «Интернет-Маркетолог: Старт Карьеры» по промокоду CAREERSTART — если хочется идти дальше, чем один запуск.' },
    ],
  }),
});
