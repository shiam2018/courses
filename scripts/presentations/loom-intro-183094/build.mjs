...
// intro video for course 183094 ("Excel с нуля: формулы, ВПР и сводные
// таблицы" — free, standalone course, soft cross-link to 297305). NOT
// uploaded to Stepik — exported as a PDF for personal use only,
  ...
  build: () => {
    ...
    const html = `
      <div class="title-wrap">
        ...
        <h1 class="big" style="color:#fff;">Excel с нуля: формулы, ВПР и сводные таблицы</h1>
        <p class="sub">Курс о том, чем обычно заканчиваются вводные курсы по Excel — ВПР, ИНДЕКС+ПОИСКПОЗ, сводные таблицы, диаграммы.</p>
      </div>
    `;
    ...
    const html = `
      ...
      <div class="avtor-content">
        ...
        <ul class="list-plain" style="margin-top:12px;font-size:13px;">
          ...
          <li>Каждый день свожу отчёты по рекламным кампаниям в Excel — на сотнях строк, не на пяти ячейках</li>
          <li>В курсе используется такой же тренировочный датасет: рекламные кампании, расходы, заявки</li>
        </ul>
      </div>
    `;
...
slides.push({
  out: 'l03-dlya-kogo',
  build: () => contentSlide('Для кого этот курс', 'Для тех, кто уже пишет СУММ, но хочет пойти дальше', `
    <ul class="list-plain" style="margin-top:14px;max-width:820px;">
      <li>Для тех, кто работает с таблицами каждый день, но не умеет ВПР и сводные таблицы</li>
      <li>Для предпринимателей и фрилансеров, которым нужно упростить работу с цифрами</li>
      <li>Бесплатный курс, 2-2,5 часа теории и практики</li>
      <li>5 модулей, тест на 10 вопросов после каждого, скачиваемый файл-тренажёр</li>
    </ul>
  `, 820),
});
...
slides.push({
  out: 'l04-programma',
  build: () => contentSlide('Программа курса', 'От первой формулы до итогового отчёта', stepsNum([
    { h: 'Формулы и ссылки', p: 'Как устроен Excel, абсолютные и относительные ссылки' },
    { h: 'Текст, даты и поиск данных', p: 'ВПР и ИНДЕКС+ПОИСКПОЗ — поиск в другой таблице' },
    { h: 'Логика, сортировка и фильтры', p: 'ЕСЛИ, СУММЕСЛИ, СЧЁТЕСЛИ, условное форматирование' },
    { h: 'Сводные таблицы', p: 'От нуля до группировки и вычисляемых полей' },
    { h: 'Диаграммы и итоговый отчёт', p: 'Выбор диаграммы + финальное задание' },
  ]), 820),
});
...
slides.push({
  ...
  build: () => wrap('', `
    <div class="content-pad">
      ...
      <h1 class="title" style="max-width:780px;">ВПР и сводные таблицы разбираются по-настоящему</h1>
      <ul class="list-plain" style="margin-top:14px;max-width:820px;">
        <li>Не «вернёмся к этому позже» — а отдельные уроки с реальными формулами</li>
        <li>Все примеры — на одном сквозном датасете рекламных кампаний</li>
        <li>Тест на 10 вопросов после каждого модуля, а не в конце курса</li>
      </ul>
      ${highlightBox('Файл-тренажёр', 'Скачиваемый Excel-файл с данными для практики и отдельным листом заданий на каждый модуль — открыли один раз, используете весь курс.')}
    </div>
  `, highlightCss),
});
...
slides.push({
  ...
  build: () => contentSlide('Как лучше проходить', 'Читайте — и сразу пробуйте на файле-тренажёре', `
    <ul class="list-plain" style="margin-top:20px;max-width:820px;font-size:15px;">
      <li>Скачайте файл-тренажёр перед первым модулем</li>
      <li>Открывайте лист «Данные» и повторяйте каждую формулу вживую, а не только читайте</li>
...
slides.push({
  ...
  build: () => recapSlide({
    title: 'В конце курса',
    cards: [
      { heading: 'Модули курса', items: ['Формулы и ссылки', 'Текст, даты и поиск', 'Логика и фильтры', 'Сводные таблицы', 'Диаграммы и отчёт'] },
      { heading: 'Что получите', items: ['Файл-тренажёр с реальными заданиями', 'ВПР и ИНДЕКС+ПОИСКПОЗ без пробелов', 'Сводные таблицы с нуля'] },
      { heading: 'Дальше', text: 'Лучший способ закрепить — применить хотя бы одну сводную таблицу к своим собственным данным.' },
    ],
  }),
});
